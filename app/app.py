...
def load_models():
    ...
    cluster_model = joblib.load(os.path.join(OUTPUT_DIR, "cluster_model.pkl"))
...
def load_data():
    ...
    if len(df) != len(embeddings):
        raise ValueError(
            f"Data mismatch: cleaned_data.csv has {len(df)} rows, "
            f"but embeddings.npy has {len(embeddings)} embeddings. "
            f"Regenerate the artifacts by running preprocessing and "
            f"autoencoder training again."
        )
...
try:
    ...
    st.error(f"Could not load model/data artifacts: {e}")
    st.stop()
...
def plot_clusters(_embeddings, _cluster_model, method="pca"):

    X = np.asarray(_embeddings)

    if X.ndim != 2:
        raise ValueError(
            f"Embeddings must be a 2D array, got shape {X.shape}."
        )

    labels = _cluster_model.predict(X)

    if len(X) != len(labels):
        raise ValueError(
            f"Cluster plot mismatch: {len(X)} embeddings but "
            f"{len(labels)} labels."
        )
    ...
    if method.lower() == "pca":
        ...
        raise ValueError(
            "method must be either 'pca' or 'tsne'."
        )
...
@st.cache_data
def cached_cluster_plot(_embeddings, _cluster_model, method="pca"):
...
def mood_to_vector(mood: str, _scaler):
    ...
    vec = np.array(
        [
            mapping.get(feature, 0.5)
            for feature in AUDIO_FEATURES
        ],
        dtype=float
    ).reshape(1, -1)

    return _scaler.transform(vec)
...
def get_recommendations(
    ...
    track = _df[
        _df["track_name"].astype(str).str.lower()
        == track_name.lower()
    ]
    ...
    row_position = track.index[0]

    X = _df[AUDIO_FEATURES].values
    query_features = X[row_position].reshape(1, -1)

    if is_autoencoder:
       
        embedding = _encoder.predict(
            query_features,
            verbose=0
        )
    ...
    if max_dist > 0:
        recs["match_score"] = (
            1 - distances[0] / max_dist
        )
    else:
        recs["match_score"] = 1.0
...
st.markdown(
    "Compare **Baseline KNN** vs **Autoencoder Embeddings** "
    "• Mood-based & Track-based recommendations"
)
...
with tab3:
    ...
    st.pyplot(
        cached_feature_distributions(df)
    )

    st.pyplot(
        cached_genre_counts(df)
    )

    st.pyplot(
        cached_correlation_heatmap(df)
    )

    st.pyplot(
        cached_popularity_distribution(df)
    )
    ...
    try:
        ...
        st.pyplot(
            cached_loss_curve(history)
        )

    except FileNotFoundError:
        st.warning(
            "Training history not found. "
            "Run autoencoder.py with history saving enabled."
        )
...
with tab4:
    ...
    st.info(
        f"Visualizing {len(embeddings):,} embeddings. "
        "Cluster labels are generated with cluster_model.predict(), "
        "so every plotted embedding has exactly one cluster label."
    )
    ...
    try:
        ...
        st.error(
            f"Could not create cluster plot: {e}"
        )
...
if mode == "Track-based":
    ...
    if selected_track:
        ...
        with st.expander("Original Track Info"):
            ...
            st.write(
                f"**Track:** {original['track_name']}"
            )

            st.write(
                f"**Artist:** {original['artists']}"
            )

            st.write(
                f"**Genre:** {original['track_genre']}"
            )

            st.write(
                f"**Popularity:** {original['popularity']}"
            )
    ...
    if st.button(
        ...
        if recs is not None and not recs.empty:
            ...
            st.success(
                f"Top recommendations for **{mood}** mood"
                + (
                    f" in **{selected_genre}**"
...
st.caption(
    "Built with Streamlit • Baseline KNN vs Autoencoder Embeddings "
